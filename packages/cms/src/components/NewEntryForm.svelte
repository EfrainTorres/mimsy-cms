<script>
  ...
  async function create() {
    ...
    const frontmatter = { [primaryField]: trimmedTitle };
    // Only inject draft:true if the schema defines a draft field
    if (schemaFields.some(f => f.name === 'draft')) {
  ...
  function isDirty() {
    return title.trim() !== '' || slugManuallyEdited || bodyContent !== (templateBody || '') ||
      Object.values(extraFields).some(v => v !== '' && v !== false && v !== undefined && !(Array.isArray(v) && v.length === 0));
  }
